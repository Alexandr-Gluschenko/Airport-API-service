...
@pytest.mark.django_db
def test_airplane_create_allowed_for_admin(api_client, admin_user, airplane_type):
...
@pytest.mark.django_db
def test_airplane_delete_authenticated_user_forbidden(api_client, user, airplane):
    api_client.force_authenticate(user=user)
